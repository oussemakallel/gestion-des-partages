...
public function searchIfShareExist($nomServ,$nomPartage)
{
    ...
    $sqlSearchPartages='select * from partages where nomPartage=? and nomServ=?' ;
    $resultSetPartages=$this->executerRequete($sqlSearchPartages,array($nomPartage,$nomServ)) ;
    $rowcountResultSetPartages=$resultSetPartages->rowCount() ;
    
    $sqlSearchdemandes='select * from demandes where nomPartage=? and nomServ=? and Not(etat=? and type=?)';
    $etat="T";
    $type="S" ;
    $resultSetDemandes=$this->executerRequete($sqlSearchdemandes,array($nomPartage,$nomServ,$etat,$type)) ;
    $rowcountResultSetDemandes=$resultSetDemandes->rowCount() ;

    return $rowcountResultSetPartages+$rowcountResultSetDemandes ;
} 
...
public function getPartage($nomPartage,$nomServ)
{
    ...
    if ($partages != null and count($partages)==1 ) {
       foreach ($partages as $partage) {
                $this->id              = $partage['id'] ;
                $this->id_demande      = $paratge['id_demande'] ;
                ...
                $this->demandeur       = (new Personnel() )->authentifier($partage['demandeur']) ;
                }
                else
                {
                $this->demandeur       ="" ;
  ...
  if ($listeCollaborateurs != null) 
    {
        ...
        foreach ($listeCollaborateurs as $collaborateur) {
          $user=new Personnel() ;
          $listColl[$i] = new Permission($user->authentifier($collaborateur['utilisateur']),$collaborateur['permission']) ;
          $i=$i+1  ;
        }
    }
